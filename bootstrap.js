...
PrefListener.prototype.uninstall = function(aReason) {
	...
	if (aReason == ADDON_UNINSTALL) { //have to put this here because uninstall fires on upgrade/downgrade too
		...
		console.log('uninstalling so delete owned branches. and delete owned prefs on UNowned branches');
		...
		Object.keys(this.watchBranches).forEach(function(branch_name) {
			...
			if (ownedPrefNames.length > 0) {
				if (ownedPrefNames.length == Object.keys(this.watchBranches[branch_name].prefNames).length) {
...
function uninstall(aData, aReason) {
	...
	if (!myPrefListener) {
		...
		console.log('in uninstall had to soft install myPrefListener')
